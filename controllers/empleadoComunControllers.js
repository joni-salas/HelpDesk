...
exports.inicio = async function (req, res, next) {
    ...
    let areas= await Area.findAll({where: {[Op.and]:[{id_area:{[Op.ne]: 3}},{id_area:{[Op.ne]: emp.id_area}}]}},)
    ...
    res.render("../views/empleadoComun",{
        empleadoComun: emp,
        areas: areas,
        clientes: clientes,
        solicitudesResueltas: solicitudesResueltas,
        passwordErrorComun: req.query.passwordErrorComun,
        passwordActualErrorComun: req.query.passwordActualErrorComun,
...
exports.cambiarContra = async function (req, res, next) {
    ...
    if(validPassword){
        if(req.body.contraNueva == req.body.contraRepeNueva){
            ...
            let passwordErrorComun= "Las contraseñas no coinciden";
            res.redirect(`../../empleado?passwordErrorComun=`+ passwordErrorComun);
        }
    }else{
        let passwordActualErrorComun= "La contraseña es incorrecta";
        res.redirect(`../../empleado?passwordActualErrorComun=`+ passwordActualErrorComun);
    }
};
...
exports.trayectoriaNoResuelta = async function (req, res, next) {
    ...
    res.redirect("../../empleado?TrayeSolicitud="+TrayeSolicitud);
